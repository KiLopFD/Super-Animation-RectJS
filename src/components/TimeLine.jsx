                    ...
                    {loading !== 0 ? data?.map((val, idx) => {
                        return (
                            <li className={`${idx !== data.length - 1 ? 'mb-10' : ''} pl-6`} key={idx}>
                                ...
                                <p className="flex items-center text-xl font-semibold font-sans text-gray-900 dark:text-white mb-3 border-2 border-[#0F1729] hover:text-amber-500 hover:border-amber-500 duration-150 ease-in px-2 rounded-2xl"><Link to={`/lesson/${categories}/${val["param"]}`} state={{ categories: categories, param: val["param"], lesson_done: lesson_done}}>{val["title"]}</Link></p>

                                {val["list"].map((_val, _idx) => {
                                    return (
                                        <p className="py-2 border-2 border-[#0F1729] pl-2 rounded-3xl text-base font-normal text-gray-500 dark:text-gray-400 hover:text-amber-500 hover:border-amber-500 duration-150 ease-in" key={_idx}><Link to={`/lesson/${categories}/${_val["param"]}`} state={{ categories: categories, param: _val["param"], lesson_done: lesson_done}}>{_val["title"]}</Link></p>
                                    )
                                })}
