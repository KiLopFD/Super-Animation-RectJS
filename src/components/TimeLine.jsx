                    ...
                    {data?.map((val, idx) => {
                        return (
                            <li className={`${idx !== data.length - 1 ? 'mb-10' : ''} pl-6`} key={idx}>
                                ...
                                <p className="flex items-center text-xl font-semibold font-sans text-gray-900 dark:text-white mb-3 border-2 border-[#0F1729] hover:text-amber-500 hover:border-amber-500 duration-150 ease-in px-2 rounded-2xl"><Link to={`/lesson/${categories}/${val["param"]}`} state={{ categories: categories, param:val["param"],}}>{val["title"]}</Link></p>
